...
async def test_Whenvideo_listingCalled_ThenReturnsempty_list(async_client, monkeypatch, tmp_path):
...
@pytest.mark.anyio
@pytest.mark.timeout(30)
async def test_stream_missing_video_returns_404(async_client, monkeypatch, tmp_path):
    flow = await AuthTestHelperAsync.register_and_login_async(async_client)
