...
IMPORTANT: These are MANUAL tests that spawn real server processes.
Run explicitly with: pytest -m manual tests/integration/test_api_integration.py
"""
...
# Skip by default in automated CI - only run when explicitly requested with -m manual
...
class BackendAPITester:
    ...
    def start_server(self, timeout=30):
        ...
        self.process = subprocess.Popen(
            cmd, cwd=self.backend_dir, env=env, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True
        )
...
class TestAuthenticationIntegration:
    ...
    async def test_user_profile_access(self, api_tester):
        ...
        # Access user profile
        async with httpx.AsyncClient() as client:
            response = await client.get(f"{api_tester.base_url}/api/auth/me", headers=api_tester.get_auth_headers())
...
class TestVocabularyIntegration:
    ...
    async def test_vocabulary_crud_operations(self, api_tester):
        ...
        async with httpx.AsyncClient() as client:
            ...
            vocab_data = {"word": "Hallo", "translation": "Hello", "language": "de", "context": "greeting"}

            response = await client.post(
                f"{api_tester.base_url}/api/vocabulary", json=vocab_data, headers=api_tester.get_auth_headers()
            )
            ...
            if response.status_code == 201:
                ...
                response = await client.get(
                    f"{api_tester.base_url}/api/vocabulary/{vocab_id}", headers=api_tester.get_auth_headers()
                )
                assert response.status_code == 200
                retrieved_vocab = response.json()
                assert retrieved_vocab["word"] == vocab_data["word"]

                print("✅ Vocabulary CRUD operations working")
            else:
    ...
    async def test_vocabulary_statistics(self, api_tester):
        ...
        async with httpx.AsyncClient() as client:
            response = await client.get(
                f"{api_tester.base_url}/api/vocabulary/stats", headers=api_tester.get_auth_headers()
            )
...
class TestVideoProcessingIntegration:
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                with open(temp_video_path, "rb") as video_file:
                    ...
                    response = await client.post(
                        f"{api_tester.base_url}/api/videos/upload", files=files, headers=api_tester.get_auth_headers()
                    )
    ...
    async def test_video_list_endpoint(self, api_tester):
        ...
        async with httpx.AsyncClient() as client:
            response = await client.get(f"{api_tester.base_url}/api/videos", headers=api_tester.get_auth_headers())
