...
async def test_Whenget_profileCalled_ThenReturnsauthenticated_user(async_client, url_builder):
    ...
    response = await async_client.get(profile_url, headers=flow["headers"])
...
@pytest.mark.anyio
@pytest.mark.timeout(30)
async def test_Whenget_profileWithoutauthentication_ThenReturnsError(async_client, url_builder):
    """Invalid input: missing authorization header yields 401."""
    profile_url = url_builder.url_for("profile_get")

    response = await async_client.get(profile_url)
...
@pytest.mark.anyio
@pytest.mark.timeout(30)
async def test_WhenUpdateLanguagesAcceptsValidPayload_ThenSucceeds(async_client, url_builder):
...
@pytest.mark.anyio
@pytest.mark.timeout(30)
async def test_Whenupdate_languagesWithduplicate_codes_ThenRejects(async_client, url_builder):
...
@pytest.mark.anyio
@pytest.mark.timeout(30)
async def test_WhenSupportedLanguagesListsKnownCodes_ThenSucceeds(async_client, url_builder):
    ...
    response = await async_client.get(languages_url)
