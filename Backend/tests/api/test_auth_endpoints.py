...
async def test_WhenRegisterCalled_ThenCreatesActiveUser(async_client):
...
@pytest.mark.anyio
@pytest.mark.timeout(30)
async def test_WhenRegisterWithMissingEmail_ThenRejects(async_client):
...
@pytest.mark.anyio
@pytest.mark.timeout(30)
async def test_WhenRegisterWithDuplicateUsername_ThenPrevents(async_client):
    """Boundary: duplicate username surfaces a 400 contract error."""
    user_data = AuthTestHelper.generate_unique_user_data()

    first_status, _ = await AuthTestHelper.register_user_async(async_client, user_data)
    assert first_status == 201

    second_response = await async_client.post("/api/auth/register", json=user_data)
    assert second_response.status_code == 400


@pytest.mark.anyio
@pytest.mark.timeout(30)
async def test_WhenLoginCalled_ThenReturnsBearerToken(async_client):
...
@pytest.mark.anyio
@pytest.mark.timeout(30)
async def test_WhenLoginWithWrongPassword_ThenRejects(async_client):
    ...
    status_code, _ = await AuthTestHelper.login_user_async(
        async_client, user_data["email"], "TotallyWrong123!"
    )
...
@pytest.mark.anyio
@pytest.mark.timeout(30)
async def test_WhenloginWithoutform_encoded_payload_ThenReturnsError(async_client):
...
@pytest.mark.anyio
@pytest.mark.timeout(30)
async def test_WhenLogoutCalled_ThenRevokestoken(async_client):
    """Happy path logout removes bearer token access."""
    flow = await AuthTestHelper.register_and_login_async(async_client)

    status_code, _ = await AuthTestHelper.logout_user_async(async_client, flow["token"])
...
@pytest.mark.anyio
@pytest.mark.timeout(30)
async def test_WhenlogoutWithoutvalid_token_ThenReturnsError(async_client):
...
@pytest.mark.anyio
@pytest.mark.timeout(30)
async def test_WhenmeCalled_ThenReturnscurrent_user_profile(async_client):
...
@pytest.mark.anyio
@pytest.mark.timeout(30)
async def test_WhenmeWithoutauthentication_ThenReturnsError(async_client):
