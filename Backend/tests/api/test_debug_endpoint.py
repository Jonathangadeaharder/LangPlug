...
async def test_WhenDebugFrontendLogsCalled_ThenSucceeds(async_http_client):
    log_entry = {
    ...
    r = await async_http_client.post("/api/debug/frontend-logs", json=log_entry)
...
async def test_WhenDebugFrontendLogs_MissingLevelCalled_ThenSucceeds(async_http_client):
    ...
    response = await async_http_client.post("/api/debug/frontend-logs", json=log_entry)
...
async def test_WhenDebughealthCalled_ThenSucceeds(async_http_client):
    r = await async_http_client.get("/api/debug/health")
    assert r.status_code == 200
    data = r.json()
