...
async def test_WhenRegisterCalled_ThenReturnsJsonResponse(async_http_client):
...
@pytest.mark.anyio
@pytest.mark.timeout(30)
async def test_WhenLoginWithWrongPassword_ThenReturnsJsonError(async_http_client):
...
async def test_WhenMeEndpointCalled_ThenReturnsJsonForBothAuthStates(async_http_client):
    ...
    unauth_response = await async_http_client.get("/api/auth/me")
