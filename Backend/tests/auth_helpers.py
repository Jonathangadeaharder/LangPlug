...
class HTTPAuthHelper:
    ...
    def register_user(self, user_data: dict[str, str] | None = None) -> tuple[int, dict[str, Any]]:
    ...
    def register_and_login(self, user_data: dict[str, str] | None = None) -> dict[str, Any]:
    ...
    def get_auth_headers(self, user_data: dict[str, str] | None = None) -> dict[str, str]:
...
class AsyncHTTPAuthHelper:
    ...
    async def register_user(self, user_data: dict[str, str] | None = None) -> tuple[int, dict[str, Any]]:
    ...
    async def register_and_login(self, user_data: dict[str, str] | None = None) -> dict[str, Any]:
    ...
    async def get_auth_headers(self, user_data: dict[str, str] | None = None) -> dict[str, str]:
        """Get authentication headers for a user."""
...
class HTTPAuthTestHelper:
    ...
    async def register_and_login_async(
        client: httpx.AsyncClient, user_data: dict[str, str] | None = None
    ) -> dict[str, Any]:
    ...
    @staticmethod
    def register_and_login(client: httpx.Client, user_data: dict[str, str] | None = None) -> dict[str, Any]:
    ...
    def verify_auth_endpoint_http(
        client: httpx.Client, auth_headers: dict[str, str], me_endpoint: str = "/api/auth/me"
    ) -> tuple[int, dict[str, Any]]:
    ...
    async def verify_auth_endpoint_http_async(
        client: httpx.AsyncClient, auth_headers: dict[str, str], me_endpoint: str = "/api/auth/me"
    ) -> tuple[int, dict[str, Any]]:
    ...
    def logout_user_http(
        client: httpx.Client, auth_headers: dict[str, str], logout_endpoint: str = "/api/auth/logout"
    ) -> tuple[int, dict[str, Any]]:
    ...
    async def logout_user_http_async(
        client: httpx.AsyncClient, auth_headers: dict[str, str], logout_endpoint: str = "/api/auth/logout"
    ) -> tuple[int, dict[str, Any]]:
...
class ContractAuthTestSuite:
    ...
    def __init__(self, client: httpx.Client | httpx.AsyncClient, is_async: bool = False):
        self.client = client
    ...
    async def run_full_auth_contract_test(self) -> dict[str, Any]:
        ...
        """
        results = {"registration": None, "login": None, "me_endpoint": None, "logout": None, "errors": []}
        ...
        try:
            ...
            if self.is_async:
                ...
                me_status, me_data = self.helper.verify_auth_endpoint_http(self.client, reg_result["headers"])
            ...
            if self.is_async:
                ...
                logout_status, logout_data = self.helper.logout_user_http(self.client, reg_result["headers"])
...
def run_contract_auth_test(client: httpx.Client | httpx.AsyncClient) -> dict[str, Any]:
    """
    ...
    if "field_types" in expected_structure:
        for field, expected_type in expected_structure["field_types"].items():
            if field in response_data:
                assert isinstance(
                    response_data[field], expected_type
...
class AuthResponseStructures:
    ...
    REGISTRATION_SUCCESS = {
        ...
        "field_types": {"id": int, "username": str, "email": str},
    }
    ...
    LOGIN_BAD_CREDENTIALS = {"status_code": 400, "required_fields": ["detail"], "field_types": {"detail": str}}
