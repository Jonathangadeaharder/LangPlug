...
class DomainLoggerService:
    ...
    def _log_with_status(
        self, logger: logging.Logger, message_success: str, message_failure: str, success: bool, log_data: dict
    ):
    ...
    def log_authentication_event(
        ...
        log_data = self._merge_log_data(
            {"event_type": event_type, "user_id": user_id, "success": success, "timestamp": self._get_timestamp()},
            additional_info,
        )
    ...
    def log_filter_operation(
        ...
        logger.info(
            f"Filter {filter_name} processed {words_processed} words, filtered {words_filtered} ({filter_rate:.2%})",
            extra=log_data,
        )
