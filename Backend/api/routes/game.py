...
async def get_game_session(
    ...
    try:
        ...
        logger.error(f"Error getting game session: {str(e)}", exc_info=True)
...
async def get_user_game_sessions(
    ...
    try:
        ...
        logger.error(f"Error getting user game sessions: {str(e)}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"Error retrieving game sessions: {str(e)}")
