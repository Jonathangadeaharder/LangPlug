...
# Force test mode for unit/integration tests
...
# Pytest hook to automatically skip manual tests unless explicitly requested
def pytest_collection_modifyitems(config, items):
    """Skip manual tests by default unless explicitly requested with -m manual"""
    # Check if user explicitly requested manual tests
    markexpr = config.getoption("-m", "")
    run_manual = "manual" in markexpr
...
def clear_service_caches():
    """
    Clear ALL lru_cache decorated functions between tests using gc introspection.
    ...
    Uses garbage collector to find all lru_cache wrappers automatically,
    so new cached functions don't require manual registration here.
    """

    def clear_all_lru_caches():
        """
        Clear known lru_cache decorated functions and registries.

        Only imports what's needed, avoiding heavy module loads.
        """
        cleared_count = 0
        ...
        # NOTE: Vocabulary services refactored to eliminate module-level singletons
        # All services now use factory functions that return fresh instances
        # No singleton reset needed - global state eliminated
...
def app(tmp_path: Path) -> Generator[FastAPI, None, None]:
    ...
    try:
        ...
        # Use temporary file-based SQLite for test isolation
        # Each test gets its own database file to prevent state pollution
        import tempfile
...
async def seeded_vocabulary(clean_database, app):
    ...
    Note: clean_database fixture runs automatically (autouse=True) before this.
    """
    ...
    # Seed the database - use context manager to ensure session is properly closed
    async with SessionLocal() as session:
        ...
        yield  # Test runs here with seeded data (session stays open)
