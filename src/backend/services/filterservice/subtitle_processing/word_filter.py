...
class WordFilter:
    ...
    def filter_word(
        ...
        """
        logger.debug("Processing word", word=word.text, user_level=user_level)
        ...
        if is_proper_name(word.text, language):
            ...
            logger.debug("Filtered proper name", word=word.text)
            return word

        # Generate lemma using spaCy (always, no fallbacks)
        try:
            lemma = lemmatize_word(word.text, language)
            logger.debug("Lemmatized", word=word.text, lemma=lemma)
        except Exception as e:
            logger.error("Lemmatization failed", word=word.text, error=str(e))
        ...
        logger.debug("Word difficulty", word=word.text, lemma=lemma, level=word_difficulty)
        ...
        logger.debug("Known check", lemma=lemma, is_known=is_known)
        if is_known:
            word.status = WordStatus.FILTERED_KNOWN
            word.filter_reason = "User already knows this word"
            logger.debug("Filtered known word", word=word.text, lemma=lemma)
            return word
        ...
        logger.debug("Level check", word_level=word_difficulty, user_level=user_level, at_or_below=is_at_or_below)
        if is_at_or_below:
            # Word is at or below user level - user has mastered this level
            word.status = WordStatus.FILTERED_AT_LEVEL
            word.filter_reason = (
                f"Word level ({word_difficulty}) at or below user level ({user_level}) - considered mastered"
            )
            word.metadata.update({"user_level": user_level, "language": language})
            logger.debug("Filtered at level", word=word.text, lemma=lemma)
            return word
        ...
        word.metadata.update({"user_level": user_level, "language": language})
        logger.debug("Active word", word=word.text, lemma=lemma, level=word_difficulty)
        return word
